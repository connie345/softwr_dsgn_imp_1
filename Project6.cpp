...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if(fabs(error) < 0.00000000000001){
        return guess;
    }
    if(error < 0){
        high_guess = guess;
...
int strCompare2(char* str1, char* str2) {
    if (whatLetter(*str2) == -1 && *str2 != 0) {
        return strCompare2(str1, str2 + 1);
    }
    else if (whatLetter(*str1) == -1 && *str1 != 0 ) {
    ...
    else if (whatLetter(*str1) < whatLetter(*str2) || *str1 == 0) {
        return -1;
    }
    else if (whatLetter(*str1) > whatLetter(*str2) || *str2 == 0) {
        return 1;
    }
    else{return 1;}
}
...
*/
Martian combineMartians(Martian x, Martian y){
    x.pennies += y.pennies;
    x.nicks += y.nicks;
    x.dodeks += y.dodeks;
    return x;
...
int numCoins(Martian x){
    return (x.pennies+x.nicks+x.dodeks);
}

Martian lessCoins(Martian x, Martian y) {
    if (numCoins(x) < numCoins(y)) {
        return x;
    } else {
        return y;
    }
...
Martian change(int cents) {
 /*   if(cents <= 0) {
    ...
    if((cents-12)>0){
        return lessCoins(combineMartians({0,0,1},change(cents-12)), combineMartians({0,1,0},change(cents-5)));
    }

    if((cents-5)>0){
        return combineMartians({0,1,0},change(cents-5));
    }
*/
...
Martian change(int cents, int nick_val, int dodek_val) {
 ...
 /*   if(cents <= 0) {
    ...
    if((cents-dodek_val)>0){
        return lessCoins(combineMartians({0,0,1},change(cents-dodek_val, nick_val, dodek_val)), combineMartians({0,1,0},change(cents-nick_val, nick_val, dodek_val)));
    }
    if((cents-nick_val)>0){
        return combineMartians({0,1,0},change(cents-nick_val));
    }
    */
}
